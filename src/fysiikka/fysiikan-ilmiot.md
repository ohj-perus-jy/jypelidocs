...
Oliosta saa kimmoisamman tai vähemmän kimmoisan, jolloin se pomppii herkemmin tai vähemmän herkästi.
...
Voit muuttaa kappaleen massaa sen `Mass`-kentän kautta, esimerkiksi:
...
`Push` kohdistaa voiman kappaleeseen yhden pelinpäivityksen ajan (1/60 s).
...
ei saisi kappaletta kulkemaan samalla nopeudella kuin `Hit`, vaan ainoastaan 1/60-osalla siitä.
...
Jos halutaan, että olio ei törmäile eli sen läpi voivat muut oliot mennä vapaasti:
...
Jos halutaan, että olio ei törmää joihinkin tiettyihin olioihin, voidaan sille lisätä `CollisionIgnoreGroup`.
...
Näin puolestaan olioon eivät vaikuta räjähdysten paineaallot:

```csharp,ignore
haamu.IgnoresExplosions = true;
```

Oliosta voidaan tehdä läpimentävä vain yhdestä suunnasta, esimerkiksi
...
Fysiikkaoliot voivat pyöriä akselinsa ympäri. Pyörimisnopeuden voi asettaa olion `AngularVelocity`-ominaisuuden avulla [ohjeen mukaisesti](../oliot/liikuttelu.md#pyorittaminen).
...
Vastaavasti pyörimisen voi taas mahdollistaa asettamalla `CanRotate`n arvoksi `true`:
