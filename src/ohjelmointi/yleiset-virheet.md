...
| `The name 'X' does not exist in the current context` | Muuttujaa tai aliohjelmaa `X` ei ole, tai se on paikallinen toisessa aliohjelmassa, tai nimi on kirjoitettu eri tavalla (isot ja pienet kirjaimet ovat eri kirjaimia). | Tarkista kirjoitusasu. Jos muuttujaa tarvitaan toisessa aliohjelmassa, ks. [Mihin koodi kirjoitetaan](mihin-koodi-kirjoitetaan.md). |
| `No overload for 'X' matches delegate ...` | Tapahtumankäsittelijän parametrit eivät vastaa tapahtumaa. Esimerkiksi törmäyskäsittelijällä pitää olla kaksi `PhysicsObject`-parametria. | Katso oikea muoto sivulta [Mihin koodi kirjoitetaan](mihin-koodi-kirjoitetaan.md#aliohjelman-parametrit-tulevat-tapahtumasta). |
...
| Ääni ei kuulu. | Tiedostomuoto ei kelpaa tai tiedosto ei ole `Content`-kansiossa. | Ks. [Äänet ja musiikki](../aanet/aanien-lisays.md). |
