...
generaattori). Lähdepuu on `../src`, joka on samaa Markdown-murretta kuin
ohj1:n ja ohj2:n materiaalit; `convert.py` kääntää sen Zensicalin
`docs/`-hakemistoksi.

**Tämä hakemisto on kopio ohj1:n `zensical/`-hakemistosta** (ohj1 `dev`,
2026-09-16). Yleiskäyttöiset korjaukset viedään käsin ohj1:een ja ohj2:een ja
sieltä tänne. [PERUSTELUT.md](PERUSTELUT.md) on ohj2:n alkuperäinen ja
kuvaa jokaisen ratkaisun perustelut, vaihtoehdot ja todennuksen; sen luvut
(esiintymät, testien määrä) ovat ohj2:n. Alla oleva teksti on ohj1:n README
sellaisenaan tätä johdantoa lukuun ottamatta; kohta "Ohj1-kohtaiset
...
- `convert.py`:
  ...
  - `ALERT_KINDS`: oppaiden merkinnät `> [!KOKEILE]`, `> [!EI TOIMI VIELÄ]`
    ...
    `kysymys`; väri ja kuvake `assets/css/admonitions.css`). Korvaavat
    TIM-wikin PNG-kuvat (play-nappi, rasti, kysymysmerkki), ja useimmiten
    lohko on pelkkä tunnusrivi, jolloin `convert_alerts` ei kirjoita tyhjää
    riviä otsikon perään. Tunnusrivin muunnos *yleiskäyttöinen*, tyypit eivät.
...
## Jypeli-ohjeiden muutokset

Mitä ohj1:n kopioon on muutettu tässä varastossa:
