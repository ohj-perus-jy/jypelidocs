...
def test_every_marked_line_is_a_real_line(printed):
    ...
    if not source_uses(r"HIGHLIGHT_[A-Z0-9]+_BEGIN"):
        pytest.skip("kirjassa ei ole korostusmerkintöjä")
...
def test_every_recording_is_drawn(printed):
    ...
    if not source_uses(r"<asciinema"):
        pytest.skip("kirjassa ei ole nauhoituksia")
...
def test_every_edit_link_points_to_an_existing_source_file(real_site):
    ...
    # Vähintään yksi linkki per luku; ohj1:ssä enemmän (SUMMARY:n ulkopuoliset sivut).
