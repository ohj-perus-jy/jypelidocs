...
# jypelidocs: oma sivusto on Jypeli-ohjeet, ulos vie Ohjelmointi 1 (ohj1:ssä toisin päin).
OHJ1 = "https://ohjelmointi1.it.jyu.fi/"
...
def test_the_menu_lists_this_site_and_ohj1(browser, base_url):
    """Oma sivusto on merkitty ja vie omalle etusivulle (ei tuotanto-osoitteeseen,
    jotta /dev/ ja localhost pysyvät itsessään); Ohjelmointi 1 vie ulos. Alasivulla
    ...
    assert items(page) == [
        {"name": "Ohjelmointi 1", "href": OHJ1, "current": False},
        {"name": "Jypeli-ohjeet", "href": home, "current": True},
    ]
    assert errors == []
...
def test_choosing_ohj1_leaves_the_site(browser, base_url):
    """Kohta on tavallinen linkki: sama välilehti, ei skriptiä välissä."""
    page, errors = open_page(browser, base_url)
    page.route(OHJ1 + "**", lambda route: route.fulfill(
        status=200, content_type="text/html", body="<title>Ohj1</title>"))
    page.click(".jyu-sites__button")
    page.click(".jyu-sites__item >> nth=0")
    page.wait_for_url(OHJ1)
    assert page.title() == "Ohj1"
    assert errors == []
