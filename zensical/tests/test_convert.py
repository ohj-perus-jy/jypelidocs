...
def ohj1_page_config(monkeypatch):
    """jypelidocs: convert.py:n NEST_UNDER ja DROP_SECTIONS ovat tyhjiä, mutta
    testit on kirjoitettu ohj1:n arvoille (siirretyt sivut, etusivun osio)."""
...
def test_dedent_headings_moves_the_heading_to_column_zero():
    """1-3 välilyöntiä otsikon edestä pois; aidan sisällä rivi on kommentti
    ja jää rauhaan, samoin neljällä sisennetty (koodia CommonMarkissakin)."""
    text = " ## Otsikko\n```bash\n # kommentti\n```\n    # koodia\n   ### Kolmas\n"
...
def test_convert_icons_wraps_a_path_arrow_written_as_a_character():
    """Suoraan kirjoitettu › saa saman kääreen kuin tagista tehty, muuten se
    jäisi täyteen tekstiväriin. Koodissa ja attribuutissa merkki ei ole polun
    erotin."""
...
def test_convert_icons_is_repeatable():
    """Valmiissa tekstissä ei ole enää tagia eikä kääreetöntä ›:tä, johon
    muunnos osuisi."""
...
def test_build_nav_accepts_star_bullets_and_uneven_indent(tmp_path, monkeypatch):
    """ohj1:n SUMMARY.md: "*"-luettelomerkki, etulinkit sarakkeessa 0 ja osat
    yhdellä välilyönnillä sisennettyinä, alasivut kolmella tai neljällä.
    Etulinkki on aina ylin taso, ja osien alasivut ovat samalla tasolla
    sisennyksen leveydestä riippumatta. Kommentoitu rivi ei ole luku."""
